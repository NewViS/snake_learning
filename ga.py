...
from dnn_model import DeepNeuralNetModel
from constants import Constants


class NEAT(BaseGameModel):

    model = None
    
    def __init__(self):
        BaseGameModel.__init__(self, "neat", "neat", "neat")
...
class NEAT_Trainer(BaseGameModel):
    ...
    def _genetic_evolution(self):
        ...
        while self.generation<3:
            ...
            self.generation += 1
            del population_size,parents,offsprings,base_offsprings,new_population

    def _pair(self, parents):
        total_parents_score = sum([x[1] for x in parents])
        pick = random.uniform(0, total_parents_score)
        parent_1 = self._roulette_selection(parents, pick)
        pick = random.uniform(0, total_parents_score)
        parent_2 = self._roulette_selection(parents, pick)
    ...
    def _save_population(self,population):
        ...
        with open('population.dat', 'wb') as output:
            pickle.dump(population, output, pickle.HIGHEST_PROTOCOL)

    def _load_object(self,filename='population.dat'):
